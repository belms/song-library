...
However, we have some issue swith database connection when application is started this way so we recomend to 
...
For example, let's try and find the artist Backstreet Boys.
...
You will notice that by the each song, on the right, there is an option to add a comment. This will lead you to
